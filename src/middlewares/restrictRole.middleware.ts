...
export default async function restrictRoleMiddleware(restrictedRoles: RoleEnum[], req: any, res: Response, next: NextFunction) {
    try {
        ...
        if (restrictedRoles.includes(user.role)) {
            console.log('check role middleware reject the user', 401);
